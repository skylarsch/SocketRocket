...
- (void)didConnect
{
    ...
    int noop = SecRandomCopyBytes(kSecRandomDefault, keyBytes.length, keyBytes.mutableBytes);
#pragma unused(noop)
...
- (void)_sendFrameWithOpcode:(SROpCode)opcode data:(id)data
{
    ...
    if (!useMask) {
        ...
        int noop = SecRandomCopyBytes(kSecRandomDefault, sizeof(uint32_t), (uint8_t *)mask_key);
#pragma unused(noop)
